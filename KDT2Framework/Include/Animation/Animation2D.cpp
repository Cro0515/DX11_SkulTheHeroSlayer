...
void CAnimation2D::ChangeAnimation(const std::string& Name)
{
	...
	//TEST
	mCurrentSequence->mBeginFuncEnable = true;
	//TEST 
	//if (mCurrentSequence->mBeginFunction && mCurrentSequence->mBeginFuncEnable)
	//{
	//	mCurrentSequence->mBeginFuncEnable = false;
	//	mCurrentSequence->mBeginFunction();
	//}
}
...
void CAnimation2D::SetShader()
{
	...
	mAnimCBuffer->SetAnimation2DReverseX(mReverse);
	mAnimCBuffer->UpdateBuffer();
}
