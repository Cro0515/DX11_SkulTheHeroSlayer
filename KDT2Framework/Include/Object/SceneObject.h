...
protected:
	...
	std::string		mName;												// 이름
	CSharedPtr<class CSceneComponent>	mRootComponent;					// 루트 컴포넌트
	std::vector<CSharedPtr<class CComponent>>	mNonComponentList;		// 씬 컴포넌트가 아닌 일반 컴포넌트 배열
	std::vector<CSharedPtr<class CComponent>>	mSceneComponentList;	// 씬 컴포넌트 배열
	float			mLifeTime = 0.f;									// 생명주기
	bool			mDamageEnable = true;								// 데미지 여부
	CSharedPtr<class CObjectSpawnPoint> mSpawnPoint;					// 스폰 포인트
	class CColliderBase* mCollision = nullptr;
...
public:
	...
	FVector3D GetCollisionMin();

	FVector3D GetCollisionMax();
