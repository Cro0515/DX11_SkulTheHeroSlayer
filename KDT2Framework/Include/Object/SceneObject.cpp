...
FVector3D CSceneObject::GetCollisionMin()
{
    return mCollision->GetMin();
}


FVector3D CSceneObject::GetCollisionMax()
{
    return mCollision->GetMax();
}
...
float CSceneObject::TakeDamage(float Attack, CSceneObject* Obj)
{
    if (!mDamageEnable)
        Attack = 0.f;

    CLog::Debug(Obj->GetName(), "가 ", this->GetName(), "에게 ", Attack, "만큼 피해를 입혔습니다.");
