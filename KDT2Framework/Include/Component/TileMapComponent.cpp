    ...
    const FVector2D& Pos)
{
    ...
    // 현재 마우스 위치의 타일 타입 받아두기
    ETileType   PrevType = mTileList[Index]->GetType();

    // 현재 마우스 위치의 타일 인자로 들어온 타입으로 변경하기
    mTileList[Index]->SetTileType(Type);
    ...
    const FVector3D& Pos)
{
    ...
    if (Frame == -1)
        mTileList[Index]->SetTileType(ETileType::Normal);
    else
        mTileList[Index]->SetTileType(ETileType::UnableToMove);
