...
protected:
	CSceneComponent* mParent = nullptr;							// 부모 컴포넌트
	EComponentRender	mRenderType = EComponentRender::None;	// 렌더 여부
	std::vector<CSharedPtr<CSceneComponent>>	mChildList;		// 자식 컴포넌트 배열
	std::string			mRenderLayerName;						// 출력 될 레이어 이름
