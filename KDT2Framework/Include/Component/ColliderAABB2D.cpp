...
void CColliderAABB2D::Update(float DeltaTime)
{
    ...
    float Xsize = mAABB.Max.x - mAABB.Min.x;
    float Ysize = mAABB.Max.y - mAABB.Min.y;


    // AABB 사이즈
    //CLog::PrintLog("X Size : " + std::to_string(Xsize) + " / Y Size : " + std::to_string(Ysize));

    // Box 사이즈
    //CLog::Debug("mBoxSizeX : ", mBoxSize.x, "mBoxSizeY : ", mBoxSize.y);
