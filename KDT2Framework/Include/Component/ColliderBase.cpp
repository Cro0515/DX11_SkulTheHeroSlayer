    ...
    CColliderBase* Dest)
{
    ...
    // 충돌상태 ON
    mCollision = eCollisionType::Block;
...
void CColliderBase::CallCollisionEnd(CColliderBase* Dest)
{
    ...
    if (mCollisionObjects.empty())
        mCollision = eCollisionType::Release;
...
void CColliderBase::CallCollisionOverlap(CColliderBase* Dest)
{
    ColliionObjectOverlap(Dest);

    mCollision = eCollisionType::Overlapped;

    if (mCollisionOverlapFunc)
        mCollisionOverlapFunc(Dest);
}
    ...
    CColliderBase* Collider)
{
    ...
    // 있을 경우 해당 노드를 가져온다.
    mCollisionObjects[Collider] = eCollisionType::Block;;
}

void CColliderBase::ColliionObjectOverlap(CColliderBase* Collider)
{
    mCollisionObjects[Collider] = eCollisionType::Overlapped;
}
...
void CColliderBase::Render()
{
    ...
    if (mCollision == eCollisionType::Block || mCollision == eCollisionType::Overlapped)
...
void CColliderBase::Refresh()
{

}

void CColliderBase::SetEnable(bool Enable)
{
    ...
    if (Enable == false)
    {
        // mCollisionObjects 순회하면서 EndFunc호출
        auto    iter = mCollisionObjects.begin();
        auto    iterEnd = mCollisionObjects.end();
