...
void CSpriteComponent::SetTexture(CTexture* Texture, int TextureIndex)
{
    mTexture = Texture;
    mTextureIndex = TextureIndex;
}
...
void CSpriteComponent::SetPlayRate(const std::string& Name, float PlayRate)
{
    mAnimation->SetPlayRate(Name, PlayRate);
}

void CSpriteComponent::FlickerStart()
{
    mFlickerValue = mFlickerInit;
}

void CSpriteComponent::SetAnimationReverseX(bool Reverse)
{
    mAnimation->SetAnimationReverseX(Reverse);
}
...
void CSpriteComponent::Update(float DeltaTime)
{
    ...
    if (0.f < mFlickerValue)
    {
        mFlickerValue -= DeltaTime * 2.0f;

        if (mFlickerValue <= 0.f)
        {
            mTint = FVector4D(1.0f, 1.0f, 1.0f, 1.0f);
            mFlickerValue = 0.f;
        }
    }
...
void CSpriteComponent::Render()
{
    ...
    mSpriteCBuffer->SetFlicker(mFlickerValue);
