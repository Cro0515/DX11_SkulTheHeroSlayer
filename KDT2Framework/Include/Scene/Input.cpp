...
void CInput::UpdateMousePos(float DeltaTime)
{
	...
	// 화면 중앙 좌표 계산
	float screenCenterX = ViewportRS.Width * 0.5f;
	float screenCenterY = ViewportRS.Height * 0.5f;

	// 마우스 위치에서 화면 중앙을 빼서 오프셋 계산
	float offsetX = mMousePos.x - screenCenterX;
	float offsetY = mMousePos.y - screenCenterY;

	// 카메라 줌 레벨에 따른 스케일 계산
	// 직교 카메라의 줌 레벨에 따라 조정
	float worldScale;
	CCameraComponent* temCameraComp = mScene->GetCameraManager()->GetViewTarget();
	if (temCameraComp == nullptr)
		worldScale = 2.f;
	else
		worldScale = temCameraComp->GetCameraZoomLevel();

	mMouseWorldPos2D.x = WorldPos.x + offsetX / worldScale * 2;
	mMouseWorldPos2D.y = WorldPos.y + offsetY / worldScale * 2;
...
void CInput::UpdateInput(float DeltaTime)
{
    ...
    case EInputSystem_Type::DInput:
		...
		{
			if (!mCtrl[EInputType::Press] && !mCtrl[EInputType::Hold])
			{
				mCtrl[EInputType::Press] = true;
				mCtrl[EInputType::Hold] = true;
			}

			else
				mCtrl[EInputType::Press] = false;
		}
		...
		else if (mCtrl[EInputType::Hold])
		{
			...
			mCtrl[EInputType::Release] = true;
		}

		else if (mCtrl[EInputType::Release])
		{
			mCtrl[EInputType::Release] = false;
		}
		...
		{
			if (!mAlt[EInputType::Press] && !mAlt[EInputType::Hold])
			{
				mAlt[EInputType::Press] = true;
				mAlt[EInputType::Hold] = true;
			}

			else
				mAlt[EInputType::Press] = false;
		}
		...
		else if (mAlt[EInputType::Hold])
		{
			...
			mAlt[EInputType::Release] = true;
		}

		else if (mAlt[EInputType::Release])
		{
			mAlt[EInputType::Release] = false;
		}
		...
		{
			if (!mShift[EInputType::Press] && !mShift[EInputType::Hold])
			{
				mShift[EInputType::Press] = true;
				mShift[EInputType::Hold] = true;
			}

			else
				mShift[EInputType::Press] = false;
		}
		...
		else if (mShift[EInputType::Hold])
		{
			...
			mShift[EInputType::Release] = true;
		}

		else if (mShift[EInputType::Release])
		{
			mShift[EInputType::Release] = false;
		}
...
void CInput::UpdateBind(float DeltaTime)
{
	...
	for (; iter != iterEnd; ++iter)
	{
		...
		{
			size_t	Size = iter->second->FunctionList[EInputType::Press].size();

			for (size_t i = 0; i < Size; ++i)
			{
				iter->second->FunctionList[EInputType::Press][i].Func(DeltaTime);
			}
		}
		...
		if (iter->second->Ctrl)
		{
			if (mCtrl[EInputType::Release])
				Verification = true;
		}

		if (iter->second->Alt)
		{
			if (mAlt[EInputType::Release])
				Verification = true;
		}

		if (iter->second->Shift)
		{
			if (mShift[EInputType::Release])
				Verification = true;
		}
			...
			iter->second->KeyHold)
		{
			...
			size_t	Size = iter->second->FunctionList[EInputType::Release].size();

			for (size_t i = 0; i < Size; ++i)
			{
				iter->second->FunctionList[EInputType::Release][i].Func(DeltaTime);
			}
		}
	}
}
